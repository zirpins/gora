...
public class IOUtils {
  ...
  private static Configuration getOrCreateConf(Configuration conf) {
    if (conf == null) {
      if (IOUtils.conf == null) {
        ...
        // GoraCassandra: enable serialization of queries with Persistent start/end keys
        GoraMapReduceUtils.setIOSerializations(IOUtils.conf, false);
      }
    }
   ...
   */
  public static <T extends PersistentBase> void serialize(OutputStream os, PersistentDatumWriter<T> datumWriter, Schema schema, Object object)
      throws IOException {
   ...
   * Serializes the field object using the datumWriter.
   */
  public static <T extends PersistentBase> byte[] serialize(PersistentDatumWriter<T> datumWriter, Schema schema, Object object) throws IOException {
   ...
   */
  @SuppressWarnings("unchecked")
  public static <K, T extends PersistentBase> K deserialize(InputStream is, PersistentDatumReader<T> datumReader, Schema schema, K object) throws IOException {
   ...
   */
  @SuppressWarnings("unchecked")
  public static <K, T extends PersistentBase> K deserialize(byte[] bytes, PersistentDatumReader<T> datumReader, Schema schema, K object) throws IOException {
   ...
   * Serializes the field object using the datumWriter.
   */
  public static <T extends PersistentBase> byte[] deserialize(PersistentDatumWriter<T> datumWriter, Schema schema, Object object) throws IOException {
    ByteArrayOutputStream os = new ByteArrayOutputStream();
    serialize(os, datumWriter, schema, object);
    return os.toByteArray();
  }
   ...
   * ceil( fields.length / 8 ) bytes are written to the output.
   *
   * @param out
  ...
  /**
   * Reads the data written by {@link #writeNullFieldsInfo(DataOutput, Object...)} and returns a
   * boolean array representing whether each field is null or not.
   *
   * @param in
  ...
  /**
   * Loads the object stored by {@link #storeToConf(Object, Configuration, String)} method from the
   * configuration under the given dataKey.
   *
   * @param conf
